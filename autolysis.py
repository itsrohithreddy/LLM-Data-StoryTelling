...
def request_llm(functions,user_content,sys_content,func_name,code_list=[],error_list=[],limit=5):
    if limit != 5:
...
content = (
    ...
    "Export/save the chart/subplot as png file. Do not use back-slash in the path while saving with python code. Use forward-slash only."
...
"Do not use back-slash in the path while saving with python code. Use forward-slash only."
)
...
# PCA AND CLUSTERING
content = (
    "Can you provide a Python code without comments that performs PCA on a dataset stored in variable ```df``` using the PCA class from sklearn.decomposition ."
    "The function should select only numerical columns from the ```df```, reduce the dimensions to 2 components, "
    "and give the explained variance ratio. Store it into a variable named as ```explained_variance``` ."
    "Transform only the numeric columns of dataset stored in variable ```df``` after performing PCA and store the transformed data in variable named ```df_pca```.Do not disturb the original data."
    "Also give python code that perform KMeans clustering on data stored in ```df_pca``` and a graph/chart which shows the clusters formed after performing KMeans."
    "Give appropriate labels and title to the chart ."
    "Use matplotlib module for graph creation and seaborn module also if necessary."
    f"save/export the chart/graph as a png file inside {sys.argv[1].split(".csv")[0]} folder."
)
...
functions = [
    {
    "name": "pca_clustering",
    "description": "Generate Python code without comments to perform PCA, plot the results, and save the chart as a PNG file.",
    "parameters": {
        "type": "object",
        "properties": {
            "python_code": {
                "type": "string",
                "description": "Generate Python code without comments."
            },
            "chart_name": {
                "type": "string",
                "description": "Name of the PNG file created."
            ...
            "dependencies": {
                "type": "string",
                "description": "Comma-separated names of the modules used in the script."
            }
        },
        "required": [
            "python_code",
...
flag , code_list , limit ,return_args = execute_llm(functions,"Do as specofied",content,'pca_clustering')

print("Python code PCA and Clustering : \n",return_args['python_code'])

# res_explained_variance = explained_variance

# print("Explained Variance : ",res_explained_variance)
# print(df_pca.head())
...
# readME['explained_variance'] = res_explained_variance
...
print("Images visualisation ended and insights created.")
print("Time taken till now : ",time.time() - start)


print("README.md generation initiated..........")
...
# markdown += "## Explained Variance\n"
# markdown += f"- **PCA**: Explained Variance Ratios: {readME['explained_variance']}\n"
...
print("Total time taken for story generation from the dataset : ",(end-start))